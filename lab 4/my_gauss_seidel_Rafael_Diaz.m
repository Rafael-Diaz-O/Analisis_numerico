function [x, iterationTable] = my_gauss_seidel_Rafael_Diaz(A, b, x0, tol, maxIter)
% MY_GAUSS_SEIDEL_FIRSTNAME_LASTNAME
% Metodo iterativo de Gauss-Seidel.
%
% [x,iterationTable] = my_gauss_seidel_firstname_lastname(A,b,x0,tol,maxIter)
%
% Entradas:
%   A       : matriz n x n
%   b       : vector n x 1
...
%   iterationTable : tabla con k, x1..xn, distance, residual
    ...
    if any(diag(A) == 0)
        error('No se puede aplicar Gauss-Seidel: hay un elemento diagonal igual a cero.');
    end
    ...
    while k < maxIter
        k = k + 1;
        xNew = xOld; % se van sobrescribiendo con los valores ya actualizados
        for i = 1:n
            s = A(i,:)*xNew - A(i,i)*xNew(i);
            xNew(i) = (b(i) - s) / A(i,i);
        end
